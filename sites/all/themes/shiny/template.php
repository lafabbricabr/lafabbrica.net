...
/* AGGIUNTE */
function shiny_file_widget($variables) {
	...
	//dpm($element);
	if (isset($element['#file']->filemime) &&  ( $element['#file']->filemime == 'image/jpeg' || $element['#file']->filemime == 'image/png') ) {
		$output .= '<div class="image-preview">';
		$output .= '<img src="' . image_style_url('medium',$element['#file']->uri) . '">';
		$output .= '</div>';
	}

	elseif(isset($element['#file']->filemime) && $element['#file']->filemime == 'text/plain') {
		$filename = file_create_url($element['#file']->uri);
		$handle = fopen($filename, "r");
		$contents = fgets($handle);
		fclose($handle);
		if( substr($contents,-3) == "jpg" || substr($contents,-3) == "png" || substr($contents,-3) == "gif") {
			$output .= '<div class="image-preview">';
			$output .= '<img src="' . $contents . '" style="max-width:220px;max-height:220px">';
			$output .= '</div>';
		}
		else {
			if(preg_match("#(?<=v=|v\/|vi=|vi\/|youtu.be\/)[a-zA-Z0-9_-]{11}#", $contents, $matches)) {
				$output .= '<div class="image-preview">';
				$output .= '<img src="http://img.youtube.com/vi/' . $matches[0] . '/sddefault.jpg" style="max-width:220px;max-height:220px">';
				$output .= '</div>';
			} else if(preg_match('#vimeo\.com\/([0-9]{1,10})#', $contents, $m)) {
				$hash = unserialize(file_get_contents("http://vimeo.com/api/v2/video/" . $m[1] . ".php"));
				$link = $hash[0]['thumbnail_large'];
				$output .= '<div class="image-preview">';
				$output .= '<img src="' . $link . '" style="max-width:220px;max-height:220px">';
				$output .= '</div>';
			}
		}
	...
	$output .= '<div class="image-widget-data file-widget form-managed-file clearfix">';
